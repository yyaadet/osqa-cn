...
def get_score_badge(user):
    ...
    if user.gold > 0 :
        BADGE_TEMPLATE = '%s%s' % (BADGE_TEMPLATE, '<span title="%(gold)s %(gold_word)s">'
        '<span class="badge1">&#9679;</span>'
        '<span class="badgecount">%(gold)s</span>'
        '</span>')
    if user.silver > 0:
        BADGE_TEMPLATE = '%s%s' % (BADGE_TEMPLATE, '<span title="%(silver)s %(silver_word)s">'
        '<span class="silver">&#9679;</span>'
        '<span class="badgecount">%(silver)s</span>'
        '</span>')
    if user.bronze > 0:
        BADGE_TEMPLATE = '%s%s' % (BADGE_TEMPLATE, '<span title="%(bronze)s %(bronze_word)s">'
    ...
    'bronze' : user.bronze,
    'gold_word' : _('gold'),
    'silver_word' : _('silver'),
    'bronze_word' : _('bronze'),
...
def diff_date(date, limen=2):
    ...
    if days > 2:
        if date.year == now.year:
            return date.strftime(_("%m-%d %H:%M").encode())
        else:
            return date.strftime(_("%Y-%m-%d %H:%M").encode())
    elif days == 2:
        return _('2 days ago')
    elif days == 1:
        return _('yesterday')
